...
import argparse
import sys
sys.path.append('/home/ubuntu/Khan/semantic_nerf')
...
def train():
    ...
    parser.add_argument('--config_file', type=str, default="/home/ubuntu/Khan/semantic_nerf/SSR/configs/SSR_room0_config.yaml", help='config file name.')
    ...
    info_mesh_file = os.path.join(mesh_dir, "info_semantic.json")
    with open(info_mesh_file, "r") as f:
        annotations = json.load(f)

    ##### This block creates the colour map for each of the semantic classes ##########    
    ...
    ############ This block is for defining the train and test images ##################
    total_num = 900
    step = 5
    ...
    ########### This block creates the training and testing samples. read the rgb, depth, semantics from the folders, process them. create a blank dictionary with 
    # keys and then append the images on the dictionary. and then finally print the training and testing sample sumamry. This block 
    # also create the noisy lables during the noisy label modes      #########
    ...
    ssr_trainer.set_params_replica() ########### This line is for scaling the images for testing and visualization purposes
    ssr_trainer.prepare_data_replica(replica_data_loader) ######## This line is for shift the data (images) from nuumpy to torch. Process and visualise the data in the tensorboard

    ##########################

    # Create nerf model, init optimizer
    ssr_trainer.create_ssr()   #### Creating the nerf model (collect the model's parameters from config file) with positional encdoer, enabling semantics and creating optimizer
    # Create rays in world coordinates
    ssr_trainer.init_rays()        

    # load_ckpt into NeRF
    ckpt_path = os.path.join(save_dir, "checkpoints", "140000.ckpt")
   ...
   ################# This block load the trimesh scene and process it. trimesh scene is the mesh.ply file. It is necessary to find appropriate 3D volumes to infer networks 
   ############ and also allign coordinates with original mesh for evaluation ############################
    level = 0.45 # level = 0
    threshold = 0.2
    draw_cameras = True
    ...
    trimesh_scene = trimesh.load(mesh_file, process=False) ############ Loading the 3D mesh file of the replica dataset

    to_origin_transform, extents = trimesh.bounds.oriented_bounds(trimesh_scene) ######### calculate the minimum oriented bounding box (OBB) of the trimesh scene. 
    print(to_origin_transform)
    ##transformation matrix that brings the OBB to the origin of the scene's coordinate system.
    T_extent_to_scene = np.linalg.inv(to_origin_transform) ### Calculate the transformation from the coordinate system of the OBB extents to the scene coordinate system
    ...
    #print(grid_query_pts)
    torch.save(grid_query_pts, '/home/ubuntu/Khan/semantic_nerf/tensor.pt')
    ...
    with torch.no_grad():
        ...
        run_MLP_fn  =  lambda pts: run_network(inputs=pts, viewdirs=torch.zeros_like(pts).squeeze(1), 
            ...
            embeddirs_fn=ssr_trainer.embeddirs_fn, netchunk=int(64*64))
    ...
    ############# This block is for calculating the semantics and vertices and faces of the mesh ###############
    ...
    ############# This block shows that there is no differences between meah.ply and mesh.canonical.ply file ################
   ...
   ################## This block is for colour prediction using the normal method #################
    ...
    ################# This block converts the color values to unsigned 8-bit integers and assign them as vertex colors to the input mesh.###################
